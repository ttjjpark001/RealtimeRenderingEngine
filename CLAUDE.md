...
- 씬 로드 시 Key/Fill/Back 3개 포인트 광원 + Orbit Directional 광원이 자동 배치됨
  ...
  - **Orbit Light**: white(1, 1, 1), intensity=6, `castShadow=true` — **Directional**, 월드 Y축 주변 궤도 회전, 45° 앙각 고정, 카메라 독립 (Phase 24)
...
- **Orbit Light 동작** (Phase 24): 매 프레임 `Engine::Update()`에서 각도 갱신(0.8 rad/s). 월드 Y축을 중심으로 회전하며 45° 앙각 고정 (카메라 독립):
  - `kElevRad = π/4` (45°), `cosElev = cos(45°) ≈ 0.707`, `sinElev = sin(45°) ≈ 0.707`
...
- **이동 속도 자동 조절**: 씬 바운딩 박스 크기에 비례하여 WASD/휠/키보드 이동 속도 조절
