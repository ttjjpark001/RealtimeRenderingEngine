...
#### RTX 5060 Ti 16GB VRAM 예산 분석

RTX 5060 Ti는 16 GB GDDR7 전용 VRAM을 갖추며, Bistro 씬 로딩 시 VRAM 부족이 발생하지 않는다.

| 항목 | 추정 VRAM | 비고 |
|------|----------|------|
| Vertex Buffer (Bistro, ~2M 정점 × 64B) | ~128 MB | |
| Index Buffer (~5.3M 인덱스 × 4B) | ~21 MB | |
| 텍스처 (343장, 평균 512×512 RGBA8 + MIP) | **~460 MB** | MIP 포함 ×4/3 |
| **Shadow Map 4096×4096 D32** | **64 MB** | 자동 결정값 (diagonal > 100m) |
| 기타 (백버퍼, Depth, 드라이버 오버헤드 등) | ~300 MB | |
| **합계** | **~1 GB** | 16 GB 대비 여유 충분 |
...
|-----|------|--------------------|----|------------|
| **RTX 5060 Ti 16GB (현재 PC)** | 16 GB | 64 MB는 전체의 0.4% | ✅ **여유** | **4096** |
...
| Intel UHD 630 (이전 PC) | ~1 GB 공유 | 831 MB 예상 → 한계 초과 | ⚠️ 비권장 | 2048 |

**RTX 5060 Ti 16GB에서 Bistro 실행 시 적용값:**

엔진 자동 결정(diagonal > 100m → 4096)이 그대로 적용된다. 코드 변경 불필요.
